    ...
    >
      ...
      {(data.conditions?.length || data.sideLink) && (
        <BlurReveal className="mt-8 md:mt-10" delay={0.3}>
          <div className="flex flex-col items-start justify-between gap-4 rounded-2xl border border-foreground/5 bg-foreground/[0.02] p-4 sm:flex-row sm:items-center md:p-5">
            {data.conditions && data.conditions.length > 0 && (
              <div className="flex flex-wrap gap-x-6 gap-y-1">
                {data.conditions.map((c) => (
                  <p key={c} className="text-sm text-foreground/50">
                    {c}
                  </p>
                ))}
              </div>
            ...
            {data.sideLink && (
              <Link
                ...
                className="inline-flex shrink-0 items-center gap-2 rounded-xl border border-accent-primary/15 bg-accent-primary/10 px-4 py-2 text-sm font-semibold text-accent-primary transition-all duration-500 ease-in-out hover:border-accent-primary/30 hover:bg-accent-primary/20"
              >
                {data.sideLink.label}
